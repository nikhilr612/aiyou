 ...
 * */
export function chunkText(text: string, max_chunk_size: number, delims: string[]): string[] {
...
export async function agentic_call(endpoint: Endpoint, history: Message[], query: string, notify: NotificationFn = default_notification_fn): Promise<string> {
  ...
  let response2;
  try {
    response2 = await llmcall(endpoint, history, query, "user", PROMPT3, base_tools);
  ...
  try {
    ...
    console.error("Document retrieval failed. Fallback to original response.\nCause: ", error);
